...
def build_episode_diagnostics(df: pd.DataFrame | None) -> dict[str, Any]:
    ...
    if {"episode_no", "total_valid_views"}.issubset(work.columns):
        ordered = work.dropna(subset=["episode_no", "total_valid_views"]).sort_values("episode_no")
        ordered["valid_view_drop"] = ordered["total_valid_views"].shift(1) - ordered["total_valid_views"]
        result["drop_off_ranking"] = _clean_records(ordered.sort_values("valid_view_drop", ascending=False).head(5)[
            [column for column in columns + ["valid_view_drop"] if column in ordered.columns]
        ].to_dict("records"))
    else:
    ...
    if total_views_median is not None and valid_rate_median is not None:
        result["high_exposure_low_valid"] = _clean_records(work[
    ...
    if {"likes", "comments"}.issubset(work.columns):
        work["interaction_score"] = work["likes"].fillna(0) + work["comments"].fillna(0)
    ...
    result["high_share_episodes"] = _top_records(work, "shares", columns)
    ...
    if result["drop_off_ranking"]:
        suggestions.append("优先复盘有效播放下滑明显的集数，核对该集开场钩子、承接关系和结尾悬念。")
    if result["high_exposure_low_valid"]:
        suggestions.append("高曝光但有效播放偏低的集数，建议检查前30秒信息密度和观众预期是否错位。")
    ...
    if result["high_interaction_episodes"] or result["high_share_episodes"]:
        suggestions.append("高互动或高分享集数可反查其情绪点、反转点和话题点，作为后续剧本复盘参考。")
